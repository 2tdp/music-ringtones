...
import static com.remi.ringtones.audiocutter.ringtonemaker.freeringtone.helpers.ConstantKt.ARTIST_REMI;
import static com.remi.ringtones.audiocutter.ringtonemaker.freeringtone.helpers.ConstantKt.MY_SAVED;
...
import com.remi.ringtones.audiocutter.ringtonemaker.freeringtone.viewcustom.ringdroid.soundfile.MP4Header;
...
public class SoundFile {
    ...
    // mDecodedSamples has the following format: {s1c1, s1c2, ..., s1cM, s2c1, ..., s2cM, ..., sNc1, ..., sNcM}
    // where s1cj is the ith sample of the jth channel (a sample is a signed short)
    ...
    private int[] mFrameGains, mFrameLens, mFrameOffsets;

    // Progress listener interface.
    public interface ProgressListener {
        /**
         * Will be called by the SoundFile class periodically with values between 0.0 and 1.0. Return true to continue loading the file or recording the audio, and false to cancel or stop recording.
         */
    ...
    public static SoundFile create(String fileName, ProgressListener progressListener) throws java.io.IOException, InvalidInputException {
        // First check that the file exists and that its extension is supported.
        File f = new File(fileName);
        if (!f.exists()) throw new java.io.FileNotFoundException(fileName);
...
//    public static SoundFile record(ProgressListener progressListener, Context context) {
...
//    }
    ...
    private void ReadFile(File inputFile) throws java.io.IOException, InvalidInputException {
        ...
        // For longer streams, the buffer size will be increased later on, calculating a rough estimate of the total size needed to store all the samples in order to resize the buffer
        // only once.
        ...
        while (true) {
            ...
            if (!done_reading && inputBufferIndex >= 0) {
                ...
                if (firstSampleData && format.getString(MediaFormat.KEY_MIME).equals("audio/mp4a-latm") && sample_size == 2) {
                    // For some reasons on some devices (e.g. the Samsung S3) you should not provide the first two bytes of an AAC stream, otherwise the MediaCodec will
                    // crash. These two bytes do not contain music data but basic info on the stream (e.g. channel configuration and sampling frequency), and skipping them
                    // seems OK with other devices (MediaCodec has already been configured and already knows these parameters).
                    extractor.advance();
                    ...
                    if (mProgressListener != null) {
                        if (!mProgressListener.reportProgress((float) (tot_size_read) / mFileSize)) {
                            // We are asked to stop reading the file. Returning immediately. The SoundFile object is invalid and should NOT be used afterward!
                            extractor.release();
                            extractor = null;
                            codec.stop();
                            codec.release();
            ...
            if (outputBufferIndex >= 0 && info.size > 0) {
                ...
                if (mDecodedBytes.remaining() < info.size) {
                    // Getting a rough estimate of the total size, allocate 20% more, and make sure to allocate at least 5MB more than the initial size.
                    ...
                    while (retry > 0) {
                        try {
                            ...
                            // setting android:largeHeap="true" in <application> seem to help not reaching this section.
                            retry--;
                        }
                    }
                    if (retry == 0) {
                        // Failed to allocate memory... Stop reading more data and finalize the instance with the data decoded so far.
                        break;
                    }
                ...
                // We could check that codec.getOutputFormat(), which is the new output format, is what we expect.
            }
            if ((info.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0 || (mDecodedBytes.position() / (2 * mChannels)) >= expectedNumSamples) {
                ...
                // Theoretically dequeueOutputBuffer(info, ...) should have set info.flags to MediaCodec.BUFFER_FLAG_END_OF_STREAM.
                // However some phones (e.g. Samsung S3) won't do that for some files (e.g. with mono AAC files), in which case subsequent
                // calls to dequeueOutputBuffer may result in the application crashing, without even an exception being thrown... Hence the second check.
                // (for mono AAC files, the S3 will actually double each sample, as if the stream was stereo. The resulting stream is half what it's supposed to be and with a much lower pitch.)
                break;
            }
        }
        ...
        for (i = 0; i < mNumFrames; i++) {
            ...
            mFrameOffsets[i] = (int) (i * (1000 * mAvgBitRate / 8) *  /*  = i * frameLens */ ((float) getSamplesPerFrame() / mSampleRate));
        }
    ...
    public void writeFile(double duration, File outputFile, int startFrame, int numFrames, Context context, ICallBackItem callBack) throws java.io.IOException {
        ...
        writeFile(duration, outputFile, startTime, endTime, context, callBack);
    }

    private void writeFile(double duration, File outputFile, float startTime, float endTime, Context context, ICallBackItem callBack) throws java.io.IOException {
        ...
        int num_out_frames = 0, num_frames = 0;
        ...
        try {
            ...
            String mime = Utils.INSTANCE.getMIMEType(outputFile.getPath());

            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                contentValues = new ContentValues();

                contentValues.put(MediaStore.MediaColumns.DISPLAY_NAME, outputFile.getName());
                contentValues.put(MediaStore.Audio.Media.ARTIST, ARTIST_REMI);
                contentValues.put(MediaStore.Audio.Media.ALBUM, MY_SAVED);
                ...
                try {
                    ...
                    id = Long.parseLong(newUri.getLastPathSegment());
                    outputStream.close();
                } catch (IOException e) {
                    callBack.callBack("failed", -1);
                ...
                contentValues = new ContentValues();
                contentValues.put(MediaStore.MediaColumns.DATA, outputFile.getPath());
                contentValues.put(MediaStore.MediaColumns.TITLE, outputFile.getName());
                contentValues.put(MediaStore.MediaColumns.SIZE, outputFile.length());
                contentValues.put(MediaStore.MediaColumns.MIME_TYPE, mime);

                contentValues.put(MediaStore.Audio.Media.ARTIST, ARTIST_REMI);
                contentValues.put(MediaStore.Audio.Media.ALBUM, MY_SAVED);
                contentValues.put(MediaStore.Audio.Media.DURATION, duration * 1000);
                contentValues.put(MediaStore.Audio.Media.IS_MUSIC, true);
        ...
        Log.d("2tdp", "writeFile: " + id);
        callBack.callBack(id, 0);
    }

    // Method used to swap the left and right channels (needed for stereo WAV files). buffer contains the PCM data: {sample 1 right, sample 1 left, sample 2 right, etc.}
    // The size of a sample is assumed to be 16 bits (for a single channel). When done, buffer will contain {sample 1 left, sample 1 right, sample 2 left, etc.}
    ...
    // Debugging method dumping all the samples in mDecodedSamples in a TSV file. Each row describes one sample and has the following format: "<presentation time in seconds>\t<channel 1>\t...\t<channel N>\n"
    // File will be written on the SDCard under media/audio/debug/ If fileName is null or empty, then the default file name (samples.tsv) is used.
