...
public class SamplePlayer {
    ...
    public int getmPlaybackStart() {
        return mPlaybackStart;
    }

    int mPlaybackStart;  // Start offset, in samples.
    ...
    public SamplePlayer(ShortBuffer samples, int sampleRate, int channels, int numSamples) {
        ...
        int bufferSize = AudioTrack.getMinBufferSize(mSampleRate, mChannels == 1 ? AudioFormat.CHANNEL_OUT_MONO : AudioFormat.CHANNEL_OUT_STEREO, AudioFormat.ENCODING_PCM_16BIT);
        // make sure minBufferSize can contain at least 1 second of audio (16 bits sample).
        if (bufferSize < mChannels * mSampleRate * 2) bufferSize = mChannels * mSampleRate * 2;

        mBuffer = new short[bufferSize / 2]; // bufferSize is in Bytes.
        mAudioTrack = new AudioTrack(AudioManager.STREAM_MUSIC, mSampleRate, mChannels == 1 ? AudioFormat.CHANNEL_OUT_MONO : AudioFormat.CHANNEL_OUT_STEREO, AudioFormat.ENCODING_PCM_16BIT, mBuffer.length * 2, AudioTrack.MODE_STREAM);
        ...
        mPlayThread = new Thread() {
            public void run() {
                ...
                while (mSamples.position() < limit && mKeepPlaying) {
                    ...
                    if (numSamplesLeft >= mBuffer.length) mSamples.get(mBuffer);
                    else {
                        for (int i = numSamplesLeft; i < mBuffer.length; i++) mBuffer[i] = 0;
